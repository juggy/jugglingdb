...
AbstractClass.find = function find(id, reload, cb) {
    ...
    this.schema.adapter.all(this.modelName, params, function (err, data) {
        ...
        if (data && data.map) {
            collection = data.map(function (d) {
                var obj = null;
                var reload = params && params.reload;
                if (constr.cache[d.id] && !reload) {
...
AbstractClass.prototype.reload = function (cb) {
    this.constructor.find(this.id, true, cb);
};
...
AbstractClass.belongsTo = function (anotherClass, params) {
    ...
    this.prototype[methodName] = function (p, cb) {
        if (p instanceof AbstractClass) { // acts as setter
            ...
            anotherClass.find(this[fk], function (err, obj) {
                if (err) return p(err);
                p(null, this.cachedRelations[methodName] = obj);
            }.bind(this));
    ...
    Object.defineProperty(cls, name, {
        ...
        get: function () {
            var f = function caller(cond, cb) {
                ...
                return targetClass.all(mergeParams(actualCond, caller._scope), function(err, a){
                    cb(err,this.cachedRelations[name] = a);
                }.bind(this));
            };
